...
class StatsCard(QFrame):
    ...
    def __init__(self, title: str, initial_val: str = "-", icon: str = "📈", parent=None):
        ...
        title_lbl.setObjectName("MutedLabel")
        title_lbl.setStyleSheet("font-size: 11px; font-weight: 600; text-transform: uppercase;")
        head_layout.addWidget(title_lbl)
        head_layout.addStretch()
        layout.addLayout(head_layout)

        self.val_lbl = QLabel(initial_val)
        self.val_lbl.setStyleSheet("font-size: 18px; font-weight: 700; color: #f8fafc;")
        layout.addWidget(self.val_lbl)

    def set_value(self, val: str, color_hex: str | None = None):
        self.val_lbl.setText(val)
        if color_hex:
            self.val_lbl.setStyleSheet(f"font-size: 18px; font-weight: 700; color: {color_hex};")
...
class StatsWidget(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        title = QLabel("CONVERSION METRICS & STATS")
        title.setObjectName("SectionTitle")
        layout.addWidget(title)

        self.lbl_subtitle = QLabel("Run conversions to track batch performance and storage savings.")
        ...
        self.card_succeeded = StatsCard("Files Succeeded", "0", "✅")
        self.card_failed = StatsCard("Files Failed", "0", "❌")
        self.card_time = StatsCard("Duration", "0.0s", "⏱️")
        self.card_in_size = StatsCard("Total Ingested", "0 B", "📥")
        self.card_out_size = StatsCard("Total Output", "0 B", "📤")
        self.card_savings = StatsCard("Storage Saved", "0 B (0%)", "💾")
        ...
        self.details_box.setStyleSheet("background-color: #151821; border-radius: 8px; padding: 12px;")
        det_layout = QVBoxLayout(self.details_box)
        det_layout.setContentsMargins(8, 8, 8, 8)
        det_layout.setSpacing(6)

        det_title = QLabel("Batch Highlights")
        det_title.setStyleSheet("font-weight: 600; color: #a5b4fc;")
    ...
    def update_stats(
        ...
        self.card_succeeded.set_value(str(success_count), "#34d399" if success_count > 0 else None)
        self.card_failed.set_value(str(fail_count), "#f87171" if fail_count > 0 else None)
        self.card_time.set_value(f"{elapsed_sec:.2f}s", "#60a5fa")
        ...
        if in_bytes > 0:
            ...
            if delta > 0:
                self.card_savings.set_value(f"-{format_bytes(delta)} ({pct:.1f}%)", "#34d399")
            elif delta < 0:
                self.card_savings.set_value(f"+{format_bytes(-delta)}", "#fbbf24")
            else:
        ...
        self.lbl_highlights.setText(
            ...
            f"• Ingestion throughput: ~{speed_mb:.2f} MB/s.\n"
            f"• Net space variation: {format_bytes(abs(delta))} "
